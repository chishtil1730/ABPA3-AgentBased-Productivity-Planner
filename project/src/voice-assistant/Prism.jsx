...
const Prism = (props) => {
    ...
    // Use a ref to store current props so the RAF loop can access them without re-running useEffect
    ...
    useEffect(() => {
        ...
        const fragment = /* glsl */ `
            ...
            // ... (rest of your existing shader logic)
            ...
            float sdOctaAnisoInv(vec3 p, float invBH, float invH, float minA){
                vec3 q = vec3(abs(p.x) * invBH, abs(p.y) * invH, abs(p.z) * invBH);
                return (q.x + q.y + q.z - 1.0) * minA * 0.5773502691896258;
            }
            ...
            void main(){
                ...
                float invBH = 1.0 / uBaseHalf;
                float invH = 1.0 / uHeight;
                float minA = min(uBaseHalf, uHeight);
                for (int i = 0; i < 60; i++) { // 💡 Reduced steps from 100 to 60 for performance
                    p = vec3(f, z); p.xz *= wob; p = uRot * p;
                    vec3 q = p; q.y += uHeight * 0.25;
                    float oct = sdOctaAnisoInv(q, invBH, invH, minA);
                    d = 0.1 + 0.2 * abs(max(oct, -q.y));
                    z -= d;
        ...
        const resize = () => {
            ...
            program.uniforms.uPxScale.value = 1 / ((gl.drawingBufferHeight || 1) * 0.1 * propsRef.current.scale);
        };
        ...
        const render = (t) => {
            ...
            // Sync Uniforms from props without restarting WebGL
            program.uniforms.iTime.value = time;
            program.uniforms.uHeight.value = currentProps.height;
            program.uniforms.uBaseHalf.value = currentProps.baseWidth * 0.5;
            program.uniforms.uGlow.value = currentProps.glow;
            program.uniforms.uHueShift.value = currentProps.hueShift;
            program.uniforms.uScale.value = currentProps.scale;
            program.uniforms.uPxScale.value = 1 / ((gl.drawingBufferHeight || 1) * 0.1 * currentProps.scale);

            // Rotation Logic (Simplified)
            if (currentProps.animationType === 'rotate' || !currentProps.animationType) {
    ...
    }, [transparent]); // 💡 Only re-run if transparency changes (rare)
